...
async function readFile(filePath, encoding) {
    return await command_1.core.fs.readFile(filePath, encoding);
}
/**
...
async function writeFile(fileName, file) {
    await command_1.core.fs.writeFile(fileName, JSON.stringify(file));
}
/**
...
async function getUpdatedFilesString(allDataFiles) {
    return Promise.all(allDataFiles.map(file => {
